...
Las etapas deben ser generales, sin entrar en demasiado detalle, aproximadamente entre 3 y 7 etapas.
...
La crítica hacia las etapas propuestas es la siguiente: {critica_actual}
La información adicional es la siguiente: {resultados_busqueda}
Para corregir la respuesta debes seguir los siguientes pasos:
    ...
    3- Anota las referencias de los resultados de búsqueda utilizados para las modificaciones realizadas. Deben enumerados en el siguiente formato para ser citados: [1] www.paginaejemplo.com, [2] www.otrapaginaejemplo.com
    4- Anota y justifica los cambios realizados en las etapas en la sección de explicación. DEBES Utilizar la información de búsqueda para apoyar tus decisiones, DEBES referenciar la información utilizada citando la sección de referencias en la sección de explicación con una notación de citas: 'Se ha incluido una sección de X [1]' -> en este caso en esta referencia habría información sobre la sección, o por qué es importante. SOLO anota lo modificado y de forma específica, evita ambigüedades como 'se han modificado las etapas siguiendo las críticas', en su lugar: 'se han modificado las etapas x e y haciendo x cambios'.
    """
)
...
def get_generador(licitacion, requisitos_adicionales, categoria_proyecto,propuesta_etapas, critica_actual, resultados_busqueda):
    instrucciones = instrucciones_agente_generador_con_revision.format(propuesta_etapas=propuesta_etapas.etapas, critica_actual=critica_actual, resultados_busqueda=resultados_busqueda)
